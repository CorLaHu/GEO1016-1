...
//    // implementation starts ...
    ...
    for(int i = 0; i < num_pts; i++){
        ...
        vector_P.insert(vector_P.end(), temp_vector_P.begin(), temp_vector_P.end());
    }
    int m = 2*num_pts, n = 12;
    Matrix P(m, n, vector_P.data());
    ...
    Vector V_last_column = V.get_column(V.cols() - 1);
    Matrix svd_M(3, 4, V_last_column.data());

    Matrix A(3, 3, 0.0);
    Vector b = svd_M.get_column(svd_M.cols() - 1);
    for(int i = 0; i < 3; i++){
        std::vector<double> temp_vector_a;
        for (int j = 0; j < 3; j++) {
            double a = svd_M(i, j);
            temp_vector_a.push_back(a);
        }
        A.set_row(i, temp_vector_a);
    }

    Vector a_1 = A.get_row(0), a_2 = A.get_row(1), a_3 = A.get_row(2);
    double rho = 1 / a_3.length();
    double rho_square = pow(rho, 2);
    cx = rho_square * dot(a_1, a_3);
    cy = rho_square * dot(a_2, a_3);
    double theta = acos(-((dot(cross(a_1, a_3), cross(a_2, a_3))) / (cross(a_1, a_3).length() * cross(a_2, a_3).length())));
    fx = rho_square * cross(a_1, a_3).length() * sin(theta);
    fy = rho_square * cross(a_2, a_3).length();
    skew = -fx * (pow(tan(theta), -1));
    ...
    // P * m = 0
    Vector Pm = P * V_last_column;
    double Pm_norml = Pm.length();
    std::cout << "P * m = " << Pm_norml << std::endl;

    // Reprojection Error
    std::vector<Vector2D> reprojection_pts;
    for(auto const &pt_3d: points_3d){
        Vector4D pt_3d_homo = pt_3d.homogeneous();
        Vector3D pt_2d_homo = svd_M * pt_3d_homo;
        Vector2D pt_2d = pt_2d_homo.cartesian();
        reprojection_pts.push_back(pt_2d);
    }
    double reprojection_distances_sum = 0.0;
    for(int i = 0; i < num_pts; i++){
        double distance_pts = distance(points_2d[i], reprojection_pts[i]);
        reprojection_distances_sum += distance_pts;
    }
    double distance_mean = reprojection_distances_sum / num_pts;
    std::cout << "Overall mean reprojection error: " << distance_mean << std::endl;
